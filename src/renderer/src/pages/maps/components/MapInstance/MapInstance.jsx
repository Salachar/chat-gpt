...
export const MapInstance = (props) => {
  onMount(() => {
    Store.setEventHandlers({
      key: props.map.name,
      handlers: {
        'add_segment': (data) => {
          Store.dispatch("add_segment", {
            key: props.map.name,
            payload: {
  ...
  const [getLightingEnabled, setLightingEnabled] = createSignal(CONFIG.is_player_screen);

  // DERIVED SIGNALS

  const getImage = () => {
    return props.mapData.image;
  };

  const getVideo = () => {
    return props.mapData.video;
  };

  const getMedia = () => {
    return props.mapData.image || props.mapData.video || null;
  };

  // OTHER BULLSHIT

  const drawPlacementsUpdateLighting = () => {
    Store.fire('draw_walls');
    Store.fire('draw_placements');
    this.updateLighting();
  }
