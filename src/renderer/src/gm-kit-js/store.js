class Store {
  ...
  dispatch (event_name, event_data) {
    ...
    // If a key is passed in, and a key is set, check for a match
    if (key && this.key && this.key !== key) return;
    // Otherwise grab the key as additional data on the payload
    ...
    if (!CONFIG.is_player_screen && this.isPlayerScreenEvent(fired_event)) {
      if (window.player_screen) {
        window.player_screen.postMessage({
          event: this.parseEvent(event_name),
          data: payload,
          key: key
        });
      }
      if (this.isPlayerScreenEventOnly(event_name)) return;
    }
    ...
    (this.__events[this.parseEvent(event_name)] || []).forEach((callback) => {
      callback(data);
    });

    // Check to see if there are registered events under the current store key
    if (this.key && this.__key_events[this.key]) {
      (this.__key_events[this.key][this.parseEvent(event_name)] || []).forEach((callback) => {
        callback(data);
      });
    }
