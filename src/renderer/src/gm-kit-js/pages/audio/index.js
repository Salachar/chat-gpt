...
class AudioContainer extends Container {
  ...
  createTrackSection (opts = {}) {
    Lib.dom.generate(['.audio_file_section', [
      [`div .title HTML=${opts.section.name}`, {
        click: (e) => {
      ...
      [`div .body ${this.data.collapse[opts.section.name] ? '.collapse' : ''}`, {
        oncreate: (node) => {
          ...
          if (opts.section.files) {
            Object.keys(opts.section.files).forEach((key) => {
              ...
              Lib.dom.generate(['input .tag_input', {
                click: (e) => {
                  e.preventDefault();
                  return false;
                },
  ...
  render () {
    Lib.dom.generate([
      ...
      ['.container_body #audio_page', [
        ['.tracks_section .section', [
          ['.title HTML=tracks'],
          ['#tracks_body .body'],
        ]],
