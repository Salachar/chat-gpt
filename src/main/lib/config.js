...
class Config {
  constructor () {
    ...
    this.path = join(this.userdata_path, this.file_name);

    IPC.on('choose_json_directory', (e) => {
      this.chooseDirectory('json', () => {
      this.createMapsFolder();
        global.shared.mainWindow.webContents.send('json_directory_chosen');
      });
    });

    IPC.on('choose_map_directory', (e) => {
      this.chooseDirectory('map', () => {
        global.shared.mainWindow.webContents.send('map_directory_chosen');
      });
    });

    IPC.on('choose_audio_directory', (e) => {
      this.chooseDirectory('audio', () => {
        global.shared.mainWindow.webContents.send('audio_directory_chosen');
      });
    });
  ...
  createMapsFolder () {
    const maps_folder = join(this.json_directory, 'maps');
