...
class AudioManager {
  ...
  sendAudio () {
    ...
    if (!list) {
      global.shared.mainWindow.webContents.send('audio_list_error', list);
    } else {
      global.shared.mainWindow.webContents.send('audio_list_loaded', list);
    }
  }

  loadAudioJSON () {
    try {
      const audio_path = join(GMConfig.json_directory, 'audio_data.json');
      let audio_data = FileHelpers.read(audio_path, { json: true });
      global.shared.mainWindow.webContents.send('audio_json_loaded', audio_data);
    } catch (e) {
      console.log(e);
      console.log('No audio_data.json file found');
      global.shared.mainWindow.webContents.send('audio_json_loaded', {});
    }
  }
  ...
  investigateFile (dir, item) {
    ...
    const file_path = join(GMConfig.audio_directory, relative_directory, item);
